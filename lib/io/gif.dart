...
class GifGenerator {
  GifGenerator({
    ...
    required int gifFrameRateFps,
        ...
        _gifFrameRateFps =
            Duration(milliseconds: (1000 / gifFrameRateFps).floor()),
  ...
  Future<void> addFrameFromSketch(Sketch sketch) async {
    ...
    final gifFrame = gif.Image.fromBytes(
        sketch.width, sketch.height, sketch.pixels!.buffer.asUint8List());
    final timeInHundredths = (_gifFrameRateFps.inMilliseconds / 10).round();
    _gifEncoder.addFrame(gifFrame, duration: timeInHundredths);
    _addedFrameCount += 1;

    print(
        '${(100 * _addedFrameCount / _totalDesiredFrameCount).round()}% gif frames generated');
  }
  ...
  Future<void> addFrame(Image frame) async {
    ...
    final gifFrame = gif.Image.fromBytes(
        frame.width, frame.height, frameBytes!.buffer.asUint8List());
    final timeInHundredths = (_gifFrameRateFps.inMilliseconds / 10).round();
    _gifEncoder.addFrame(gifFrame, duration: timeInHundredths);
    _addedFrameCount += 1;
