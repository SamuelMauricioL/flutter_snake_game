...
import 'package:snake_game/io/gif.dart';
...
class ProcessingDemoState extends State<ProcessingDemo>
    with SingleTickerProviderStateMixin
    implements SketchDemo {
  ...
  Future<void> _doTakeScreenshot(File file, Completer<void> completer) {
    ...
    if (!_sketch!.isLooping) {
      if (_sketch!.publishedFrame == null) {
        print(
            "Can't take screenshot because the Sketch isn't looping and frame is available.");
  ...
  Future<void> _doStartTakingScreenshotFrames({
    ...
    _frameNameTemplate = nameTemplate.endsWith(format.extension)
        ? nameTemplate
        : '$nameTemplate.${format.extension}';
    _frameFormat = format;
  ...
  Future<void> _onSketchFrameAvailable(Image frame) async {
    if (_screenshotCompleter != null) {
      ...
      // another opportunity to add to the GIF. We need to complete it early.
      final timeToSave =
          (_gifGenerator!.isDoneAddingFrames || !_sketch!.isLooping) &&
              !_gifGenerator!.hasSavedToFile;
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return AnimatedAlign(
          alignment: _screenshotCountdown.isCountingDown
              ? Alignment.topCenter
              : Alignment(0.0, -1.4),
...
class _CountdownController with ChangeNotifier {
  ...
  Duration get countdownDuration => _countdownTotal != null
      ? Duration(
          milliseconds:
              (_countdownTotal!.inMilliseconds * countdownPercent).round())
      : Duration.zero;

  double get countdownPercent =>
      _countdownTotal != null ? _timerController.value : 0.0;
